...
using namespace std ;
PlayerUI::PlayerUI(int treasury) : city(*new City(treasury)), isAI(false), aiLevel(AiLevel::NONE), name("Player"){
...
int PlayerUI::getShields() const{
    return city.getShields();
};

int PlayerUI::getScore(GameUI& game) const{
...
int PlayerUI::getPlayerChoice(int max){
    int choice;
    bool first_try = true;
    if (max <1){
        return 0;
...
void PlayerUI::play(GameUI& game){
...
//     return false;
// }
void PlayerUI::addWonderToCity(Wonder* wonder) {
...
void PlayerUI::chooseWonder(std::vector<Wonder*>& availableWonders) {
