...
void DiscardAdversaryCard::effect(Game& game){
    ...
    std::cout << game.getTurnPlayer().getName() << " choisit un batiment a supprimer de la ville de l'adversaire (défaut = 1) :" << std::endl;
    for (size_t i = 0; i < buildings_to_discard.size(); i++){
        std::cout << "\t" <<i+1 << " : " << buildings_to_discard[i]->getName() << std::endl;
    }
    int choice = getIntInput();
    if (choice < 1 || static_cast<size_t>(choice) > buildings_to_discard.size()){
